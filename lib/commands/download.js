...
module.exports = {
  ...
  run: function() {
    ...
    let downloadPaths = config.downloadPaths || ['translations'];
    ...
    return new Promise((resolve, reject) => {
      ...
      api.exportTranslations(config.projectName).then(result => {
        ...
        .on('entry', entry => {
          ...
          let relativePath = entryPath.length === 2 ? entryPath[1].substr(1) : null;
          let isThisProject = entry.path.split("/").some((path) => path === folderName);
          let shouldDownload = relativePath && isThisProject && downloadPaths.some((path) => relativePath.includes(path)) && !entry.path.includes("en-US.yaml");
