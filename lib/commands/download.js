      ...
      return api.exportTranslations(config.projectName).then(result => {
        ...
        .on('entry', entry => {
          ...
          if(isThisProject && entry.type !== 'Directory'){
            ...
            this.ui.writeLine(chalk.green(`Downloaded file ${entry.path} from Crowdin`));
          } else {
            entry.autodrain();
          }
        })
