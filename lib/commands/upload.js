...
module.exports = {
  ...
  run: function() {
    ...
    this.ui.startProgress(`Uploading source assets to Crowdin's ${folderName} folder`);

    return api.updateFile(config.projectName, toUpload, null, folderName).then(() => {
      this.ui.stopProgress();
      this.ui.writeLine(chalk.green(`Source assets on Crowdin's ${folderName} folder were successfully updated`));
    }).catch(error =>  {
      this.ui.stopProgress();
