...
function printReport(report, ui) {
  ...
  Object.keys(report).forEach((key) => {
    ui.writeLine(chalk.bgRed(`#### ${key} ####\n`));
    report[key].forEach((dup) => {
      ui.writeLine(`L${dup.line} '${chalk.cyan(dup.key)}' diff with ${chalk.blue(dup.against)}:\n`);
  ...
  ui.writeError(`\n Found ${chalk.red(count)} duplicated keys, with different values`);
}
