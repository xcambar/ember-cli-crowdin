    ...
    return new Promise((resolve, reject) => {
      waitAndExport(config.projectName).then(result => {
        ...
        if (!fs.existsSync(tmpDir)){
          fs.mkdir(tmpDir, ()=> {
            this.ui.writeLine(chalk.green(`Temp directory was created: ${tmpDir}`));
          });
        }
        ...
        .pipe(unzip.Parse())
        .on('entry', (entry) => {
          if (entry.type === 'File' && entry.path.includes('en-US.yaml')) {
        ...
        .on('close', () => {
          rimraf(tmpDir, () => {
            ...
            if(report.count !== 0) {
              ...
              // Disable this for now, the previous version outdoorsy-translations "en" doesn't exist anymore for some reason
              // reject();
            }
